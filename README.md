...
ログインの合言葉は平文では保存せず、**PBKDF2-SHA256（20万回）＋ランダムsalt** の
「検証ハッシュ」だけを `script.js` の `PASS_VERIFIER` に置いています。入力を同じ方式で
ハッシュ化して照合するため、**ソースを見ても元の合言葉は分からず、総当たりも非常に重い**です。

- **合言葉を変える**：`tools/passcode.html` をブラウザでローカルに開き、新しい合言葉を入力 →
  出力された `PASS_VERIFIER = { ... }` を `script.js` 冒頭に貼り替えてコミット。
- 強い合言葉にするほど安全になります（弱い単語は総当たりされ得るため避ける）。

> 注意：この方式は「サイト経由のアクセス」を実用上守りますが、リポジトリが公開の間は
> `images/` などコミット済みアセットは直接見られます。完全な保護はサーバー移行時に。

## カスタマイズ

- **合言葉を変える**：上記「ログインの合言葉」を参照（`tools/passcode.html` で生成）
