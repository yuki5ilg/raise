...
「動画」セクションに、YouTubeの限定公開動画を埋め込み表示します。

- 表示する動画は `data/videos.json` の `videos` に追加します。

```json
{
  "videos": [
    { "title": "練習の様子①", "url": "https://youtu.be/xxxxxxxxxxx" },
    { "title": "試合ハイライト", "url": "https://www.youtube.com/watch?v=yyyyyyyyyyy" }
  ]
}
```

- `url` は通常の共有リンクでOK（`youtu.be/...` / `watch?v=...` / `/shorts/...` いずれも可）。
- 限定公開動画は「リンクを知っている人だけ」が見られる仕組みのため、**自動取得はできません**
  （公開リストやAPIに出てこないのが限定公開の仕様）。リンクを上記に登録してください。
- 埋め込みが表示されない場合は、その動画の YouTube 設定で「埋め込みを許可」がONか確認してください。
- このサイト全体が合言葉ログインの内側にあるため、合言葉を知る人だけが動画を見られます。
